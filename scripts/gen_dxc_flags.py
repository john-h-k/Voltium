# if you replace the string below, make sure to
# change <value> <value> to <value><value> (so just find-replace "> <" with "><")
# make sure descriptions are on the same line
# remove any (default) parts

a = """//  DXC Compiler flags
// Common Options:
...
//   -pack-prefix-stable Pack signatures preserving prefix-stable property - appended elements will not disturb placement of prior elements
...
#transrights n all that 😎. this method means translate tho
def trans(s, b):
    if len(s) < 6:
        b = b.title().replace(" ", "")
        if "(" in b:
            return b.replace(b[b.index("("):b.index(")") + 1], "")
        else:
            return b.replace("-", "").replace(".", "")
    else:
        return s[1:].replace("-", " ").title().replace(" ", "").replace(".", "")

with open("dxc_flags.g.cs", "w") as file:
    for line in a.split("\n"):
        oline = line
        line = line[2:].strip()
        ...
        if "<" in line[0] and ">" in line[0]:
            haseq = False
            ln = line[0].split("<")[0]
            args = [f"object {i[:i.index('>')]}" for i in oline.split("<")[1:]]
            argsstr = ", ".join(args)
            if ln[-1] == "=":
                haseq = True
                ln = ln[:-1]
            desc = desc.replace("=<", " ").replace("<", " ").replace(">", " ")
            argstring = f"{ln}=\\\"{{{args[0].split(' ')[1]}}}\\\""
            file.write(f"\n/// <summary>\n/// {desc}\n/// </summary>\n")
            file.write(f"\n\t\tpublic static Flag {trans(ln, desc)}({argsstr}) => $\"{argstring}\";")
        else:
            file.write(f"\n/// <summary>\n/// {desc}\n/// </summary>\n")
            file.write(f"\t\tpublic static Flag {trans(line[0], desc)} {{ get; }} = \"{line[0]}\";")
